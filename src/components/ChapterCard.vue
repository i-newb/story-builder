<template>
  <div class="chapter-card">
    <div class="chapter-header" :class="{ open: isOpen }" @click="isOpen = !isOpen">
      ...
      <span class="chapter-toggle"><el-icon>
          <ArrowDown />
        </el-icon></span>
    </div>
    ...
    <div class="chapter-body" :class="{ open: isOpen }">
      ...
      <input type="text" class="mini-input chapter-numeral" :value="chapter.numeral"
        @input="chapter.numeral = $event.target.value" />

      <Draggable class="block-list" :list="chapter.blocks" :item-key="getBlockDragKey" handle=".block-header"
        ghost-class="block-ghost" chosen-class="block-chosen" drag-class="block-dragging">
        <template #item="{ element: block, index: blockIndex }">
          <div class="block-item">
            <div class="block-header" :class="{ open: openBlocks.has(getBlockDragKey(block)) }"
              ...
              <span class="block-type-dropdown" @click.stop>
                <el-dropdown trigger="click" @command="newType => changeBlockType(blockIndex, newType)">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item v-for="(label, type) in BLOCK_LABELS" :key="type" :command="type"
            ...
            <div class="block-body" :class="{ open: openBlocks.has(getBlockDragKey(block)) }">
              <BlockEditor :block="block" :characters="characters" />
            </div>
          </div>
        </template>
      </Draggable>
...
function deleteChapter() {
  ...
  if (props.chapters.length <= 1) {
    ElMessageBox.alert('至少保留一个章节', '提示', {
      confirmButtonText: '关闭',
    })
    return
  }

  ElMessageBox.confirm('确定要删除这个章节吗？章节内的所有内容块也会一起删除。', '删除章节', {
...
function deleteBlock(blockIndex) {
  ElMessageBox.confirm('确定要删除这个内容块吗？', '删除内容块', {
    confirmButtonText: '删除',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(() => {
