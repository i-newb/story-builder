import { escHtml, nl2br, hexToRgba, normalizeHexColor, NUMERALS } from './index.js'
...
export function generateHTML(story) {
  ...
  const safeAccent = normalizeHexColor(accent, '#C07858')
  const safeBg = normalizeHexColor(bg, '#F7F3EE')
  const accentSoft = hexToRgba(safeAccent, 0.15)
  const charColorVars = characters
    .map((char, index) => `  --char-color-${index}: ${normalizeHexColor(char.color, '#E0F0E4')};`)
    .join('\n')
  ...
  return `<!DOCTYPE html>
    ...
    <style>
      ...
      :root {
        ...
        --a-color: ${normalizeHexColor(characters[0]?.color, '#E0F0E4')};
        --b-color: ${normalizeHexColor((characters[1] || characters[0])?.color, '#FFF0E8')};
    ${charColorVars}
        --ink: #2D2B2A;
        --ink-2: #6B6460;
        --border: #3D3A38;
